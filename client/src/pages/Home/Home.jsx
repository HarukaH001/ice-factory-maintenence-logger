      ...
      data.forEach((ele, i) => {
        ...
        if (ele.part) {
          ...
          procData.push({
            "#": ele.lid,
            "ว/ด/ป":
              (ele.dd < 10 ? "0" + ele.dd : ele.dd) +
              "/" +
              (ele.mm < 10 ? "0" + ele.mm : ele.mm) +
              "/" +
              ele.yyyy,
            เวลา:
              (ele.hour < 10 ? "0" + ele.hour : ele.hour) +
              ":" +
              (ele.min < 10 ? "0" + ele.min : ele.min) +
              " น.",
            บ่อ: ele.location,
        ...
        target.forEach((tg, i) => {
          ...
          keyword.forEach((kw) => {
            if (ele[tg]) {
              const temp = ele[tg] + "";
              if (kw.length > 1) {
                weight += stringSimilarity.compareTwoStrings(
                  kw.toLowerCase(),
                  temp.toLowerCase()
                );
              } else {
    ...
    <div className="Home">
      <div className="container">
        <div className="home-container">
          ...
          <Form className="search-form" onSubmit={(e) => {e.preventDefault(); setSearched(true)}}>
      ...
      >
        <Toast.Body>
          <b style={{ fontSize: "18px" }}>ลบรายการซ่อม #{del} เรียบร้อยแล้ว</b>
        </Toast.Body>
      </Toast>
    </div>
  );
